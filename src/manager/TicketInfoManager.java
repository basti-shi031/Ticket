...
public class TicketInfoManager {
    ...
    //解析数据
    public static TicketInfo parseTicketInfo(String content, String filePath) {
        ...
        Date date = new Date();
        //todo 处理日期
        TicketInfo ticketInfo = new TicketInfo(code, number, hashcode, date, filePath);
        return ticketInfo;
    }

    //处理得到的信息
    public static void transactTicketInfo(TicketInfo ticketInfo) {
        //流程：
        ...
        DBUtil.init();
        try {
            TicketInfo originTicketInfo = DBUtil.isExist(ticketInfo);
